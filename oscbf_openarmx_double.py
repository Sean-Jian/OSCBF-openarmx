...
class OpenArmXDualController(mujoco_viewer.CustomViewer):
    ...
    def compute_arm_torque(self, arm_side="left"):
        ...
        Kp_pos = np.diag([200]*3); Kd_pos = np.diag([30]*3)
        Kp_ori = np.diag([15]*3);  Kd_ori = np.diag([5]*3)
        ...
        # 纯关节空间控制（不用笛卡尔项，避免坐标系不匹配导致乱动）
        Gamma_nom = -Kpj*(q - q_des) - Kdj*qdot
    ...
    def runBefore(self):
        ...
        # ---- 目标模式选择 ----
        # mode = "L_pose"      : 保持 L构型（默认）
        # mode = "ball"        : 右臂往右转（joint1 增大），验证 OSCBF 绕障
        mode = "ball"

        if mode == "ball":
            ball_pos = self.get_ball_position()
            # 右臂 joint3 往球方向转
            q_right_now = self.get_current_qpos("right")
            self.right_joint_start = q_right_now.copy()
            self.right_joint_end   = q_right_now.copy()
            self.right_joint_end[2] = 1.0
            self.right_move_steps  = int(8.0 / self.dt)
            self.right_Kpj = 1200
            self.right_Kdj = 120

            # 左臂 joint3 往本体方向转
            q_left_now = self.get_current_qpos("left")
            self.left_joint_start = q_left_now.copy()
            self.left_joint_end   = q_left_now.copy()
            self.left_joint_end[2] = 1.0
            self.left_move_steps  = int(8.0 / self.dt)
            self.left_Kpj = 1200
            self.left_Kdj = 120

            print(f"右臂 joint3 → {np.degrees(self.right_joint_end[2]):.1f}°（往球）  小球={np.round(ball_pos,3)}")
            print(f"左臂 joint3 → {np.degrees(self.left_joint_end[2]):.1f}°（往本体）")
        else:
            self.right_move_steps = 0
            print("目标模式：保持 L构型")

        print("左臂目标末端:", np.round(self.left_target_ee_pos, 3))
        print("右臂目标末端:", np.round(self.right_target_ee_pos, 3))

    def runFunc(self):
        if self.index >= self.num_steps:
            return

        # 关节空间插值更新右臂目标（ball 模式）
        if hasattr(self, 'right_move_steps') and self.right_move_steps > 0:
            t = min(self.index / self.right_move_steps, 1.0)
            q_current_right = self.get_current_qpos("right")
            q_target = q_current_right.copy()
            q_target[2] = self.right_joint_start[2] + t * (self.right_joint_end[2] - self.right_joint_start[2])
            self.right_ik_joint_angles = q_target
            ee_now = self.get_end_effector_position("right")
            if ee_now is not None:
                self.right_target_ee_pos = ee_now.copy()
            ori_now = self.get_end_orientation("right")
            if ori_now is not None:
                self.right_target_ee_orientation = ori_now.copy()
            if self.index % 200 == 0 and t < 1.0:
                ball_pos = self.get_ball_position()
                dist = np.linalg.norm(ee_now - ball_pos) if ee_now is not None else -1
                print(f"[右臂 {self.index}] 进度={t*100:.1f}%  joint3={np.degrees(q_target[2]):.1f}°  距小球={dist:.3f}m")

        # 关节空间插值更新左臂目标（body CBF 验证）
        if hasattr(self, 'left_move_steps') and self.left_move_steps > 0:
            t = min(self.index / self.left_move_steps, 1.0)
            q_current_left = self.get_current_qpos("left")
            q_target_l = q_current_left.copy()
            q_target_l[2] = self.left_joint_start[2] + t * (self.left_joint_end[2] - self.left_joint_start[2])
            self.left_ik_joint_angles = q_target_l
            ee_now_l = self.get_end_effector_position("left")
            if ee_now_l is not None:
                self.left_target_ee_pos = ee_now_l.copy()
            ori_now_l = self.get_end_orientation("left")
            if ori_now_l is not None:
                self.left_target_ee_orientation = ori_now_l.copy()
            if self.index % 200 == 0 and t < 1.0:
                # 计算左臂到本体 box 的距离
                body_bid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, "robot_body_collision_body")
                body_pos = self.data.body(body_bid).xpos if body_bid != -1 else self.body_box_center
                dist_body = np.linalg.norm(ee_now_l - body_pos) if ee_now_l is not None else -1
                print(f"[左臂 {self.index}] 进度={t*100:.1f}%  joint3={np.degrees(q_target_l[2]):.1f}°  "
                      f"末端={np.round(ee_now_l,3) if ee_now_l is not None else 'N/A'}  距本体={dist_body:.3f}m")
